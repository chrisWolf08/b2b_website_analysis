CREATE OR REPLACE TABLE `neogen-ga4-export.reporting_enterprise.site_search_daily`
AS
WITH base_events AS (
  SELECT
  ...
  WHERE _TABLE_SUFFIX BETWEEN '20250805' 
...
pageview_events AS (
  SELECT
    ...
    page_location
  FROM base_events
  WHERE event_name IN ('page_view','screen_view','view_item')  -- adjust if you want only page_view
),
...
LEFT JOIN unique_pageviews u
  ON s.event_date = u.event_date
  AND s.page_location = u.page_location
GROUP BY date, market_id, hostname, search_term, page_location, pageviews
